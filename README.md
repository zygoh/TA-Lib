...
- **母婴科普**：`zygo-skills/skills/maternal-post-flow/SKILL.md` 是编排真源（选题 → 权威取证 → 合规前置 → 并行清洗/配图 → Stage 2.5 双重校验 → **发 TG（主渠道）**→ 默认入公众号草稿 → 更新 `MEMORIES.md`）。`maternal-distribute` 对应 **`POST /maternal-mcp/distribute`**（`title` + `text` + `image` 必传；`digest` 可选）；**默认开启** Stage 3.5 `maternal-wechat-draft` 对应 **`POST /maternal-mcp/wechat-draft`**（同稿同图入公众号草稿箱；`publish_wechat=false` 可关）。Stage 4 仅提交 `skills/maternal-post-flow/MEMORIES.md`（选题 + 配图风格）。
...
- **`POST /maternal-mcp/wechat-draft`** — 表单 `title`、`content_html`、`image`（必填）；可选 `digest`、`author`、`content_source_url`。对应 `maternal-wechat-draft`（默认开启 Stage 3.5；`publish_wechat=false` 可关）。
